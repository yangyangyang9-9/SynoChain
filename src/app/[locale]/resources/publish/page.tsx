    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 pt-16">
        <div className="glass rounded-2xl p-12 text-center max-w-md">
          ...
          <p className="text-sm text-gray-500 mb-6">{t('resources.publishTitle')}</p>
    ...
    <div className="min-h-screen pt-24 pb-16">
      <div className="max-w-2xl mx-auto px-4">
        <div className="glass rounded-2xl p-8">
          <h1 className="text-2xl font-bold gradient-text mb-8 text-center">{t('resources.publishTitle')}</h1>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-2">{t('resources.titleField')}</label>
              <input
                type="text"
                {...register('title', { required: t('resources.titleRequired') })}
                placeholder={t('resources.titleField')}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-2">{t('resources.description')}</label>
              <textarea
                {...register('description', { required: t('resources.descriptionRequired') })}
                placeholder={t('resources.description')}
                rows={5}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-2">{t('resources.contact')}</label>
              <input
                type="text"
                {...register('contact', { required: t('resources.contact') })}
                placeholder={t('resources.contact')}
                className="w-full px-4 py-3 rounded-xl bg-white/[0.05] border border-white/10 text-white placeholder-gray-600 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/30 transition-all"
              />
            ...
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-2">{t('resources.country')}</label>
              <select
                {...register('country', { required: t('resources.country') })}
                className="w-full px-4 py-3 rounded-xl bg-white/[0.05] border border-white/10 text-gray-300 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/30 transition-all"
              >
                <option value="">{t('resources.country')}</option>
                {countryList.map((c) => (
