...
export default function DashboardPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 pt-16">
        <div className="glass rounded-2xl p-12 text-center max-w-md">
          ...
          <p className="text-sm text-gray-500 mb-6">您需要登录后才能访问控制台</p>
          <Link
            href="/login"
  ...
  const tabs: { key: Tab; label: string }[] = [
    ...
    { key: 'recommendations', label: 'AI推荐' },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {actionMsg && (
          <div className={`mb-6 p-3 rounded-lg text-sm ${
            actionMsg.includes('成功') ? 'bg-emerald-500/10 border border-emerald-500/30 text-emerald-400' : 'bg-red-500/10 border border-red-500/30 text-red-400'
          }`}>
            {actionMsg}
          </div>
        )}
        ...
        {loading ? (
          ...
          <>
            {activeTab === 'resources' && (
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-semibold text-white">我的资源</h2>
                  <Link
            ...
            {activeTab === 'demands' && (
              <div>
                <div className="flex items-center justify-between mb-6">
                  <h2 className="text-xl font-semibold text-white">我的需求</h2>
                  <Link
                ...
                {demands.length === 0 ? (
                  <div className="glass rounded-xl p-12 text-center">
                    <p className="text-gray-500 mb-4">暂无需求</p>
            ...
            {activeTab === 'recommendations' && (
              <div>
                <h2 className="text-xl font-semibold text-white mb-6">AI 推荐</h2>
                {recommendations.length === 0 ? (
                  <div className="glass rounded-xl p-12 text-center">
                    <p className="text-gray-500 mb-4">暂无AI推荐</p>
                    <p className="text-xs text-gray-600">
                      发布更多资源或需求后，AI 将自动为您匹配商业机会
                    </p>
                  </div>
                ) : (
                    ...
                    {recommendations.map((rec) => (
                      <div key={rec.id} className="glass rounded-xl p-6 gradient-border">
                        <div className="flex items-center justify-between mb-4">
                          <span className="text-lg font-bold gradient-text">{rec.match_score}% 匹配</span>
                        ...
                        <div className="mt-4 flex items-center justify-between">
                          ...
                          <button className="text-xs text-cyan-400 hover:text-cyan-300">查看详情</button>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {activeTab === 'subscription' && (
              <div>
                <h2 className="text-xl font-semibold text-white mb-6">订阅管理</h2>
                {user?.is_subscribed ? (
                  <div className="glass rounded-xl p-8">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <div>
                        <p className="text-lg font-semibold text-white">Premium 会员</p>
                        <p className="text-sm text-gray-400">您已订阅高级版，可查看全部优质供应链数据</p>
                      </div>
                    </div>
                    <Link
                      href="/resources"
                  ...
                  <div className="glass rounded-xl p-8 text-center">
                    <p className="text-gray-400 mb-2">您当前使用的是免费版</p>
                    <p className="text-sm text-gray-600 mb-6">订阅 Premium 解锁 5,000+ 优质供应链资源与优先匹配</p>
                    <Link
                      href="/subscription"
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="glass rounded-2xl p-8 max-w-sm w-full text-center">
            ...
            <p className="text-sm text-gray-400 mb-6">此操作不可撤销，确定要删除吗？</p>
...
function EditModal({ type, id, token, onClose, onSuccess }: {
  ...
  const onSubmit = async (data: Record<string, string>) => {
    ...
    try {
      ...
      setServerError(err instanceof Error ? err.message : '更新失败')
    }
  }
        ...
        {loading ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-400 mb-1.5">国家</label>
                <select
